...
void main() 
{
    ...
    for(int i = 0; i < num_of_lights; i++) 
    {
        ...
        diffuse = light_color[i] * material_color * max(dot(frag_normal, light_direction), 0.0);
    
        vec3 reflection = normalize(reflect(light_direction, frag_normal));
        vec3 view_direction = normalize(camera_position - frag_pos);
        specular = light_color[i] * material_color * clamp(pow(dot(reflection, view_direction), material_shininess), 0.0, 1.0);
    }
